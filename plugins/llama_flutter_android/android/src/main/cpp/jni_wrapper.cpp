...
Java_com_write4me_llama_1flutter_1android_LlamaFlutterAndroidPlugin_nativeDetectGpu(
    ...
    appInfo.pApplicationName = "PrivateLM";
    ...
    if (result != VK_SUCCESS) {
        LOGE("nativeDetectGpu: Failed to create Vulkan instance: %d", result);
        return nullptr;
    }
    ...
    if (result != VK_SUCCESS || deviceCount == 0) {
        LOGI("nativeDetectGpu: No Vulkan devices found");
    ...
    if (!hasCompute) {
        LOGI("nativeDetectGpu: Vulkan device '%s' has no compute queue", props.deviceName);
        vkDestroyInstance(instance, nullptr);
        return nullptr;
    }

    // Get device memory properties for VRAM estimation
    VkPhysicalDeviceMemoryProperties memProps;
    vkGetPhysicalDeviceMemoryProperties(physicalDevice, &memProps);

    // Find the largest device-local heap (this is UMA on Android, so it's system RAM)
    int64_t maxHeapSize = 0;
    for (uint32_t i = 0; i < memProps.memoryHeapCount; i++) {
        if (memProps.memoryHeaps[i].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
            if ((int64_t)memProps.memoryHeaps[i].size > maxHeapSize) {
                maxHeapSize = (int64_t)memProps.memoryHeaps[i].size;
            }
        }
    }
    ...
    // Fill output stats
    jlong stats[2] = {
        static_cast<jlong>(props.apiVersion),
        maxHeapSize
    };
    ...
    jstring gpuName = env->NewStringUTF(props.deviceName);
    LOGI("nativeDetectGpu: Found Vulkan device: %s (api=0x%x, mem=%lld MB)",
         props.deviceName, props.apiVersion, (long long)(maxHeapSize / (1024 * 1024)));
    return gpuName;
}
...
Java_com_write4me_llama_1flutter_1android_LlamaFlutterAndroidPlugin_nativeLoadModel(
    ...
    // Check file accessibility before loading
    FILE *f = fopen(model_path, "rb");
    if (!f) {
        LOGE("Cannot open model file: %s (errno=%d: %s)", model_path, errno, strerror(errno));
        jclass exception = env->FindClass("java/lang/RuntimeException");
        env->ThrowNew(exception, "Cannot open model file");
        env->ReleaseStringUTFChars(path, model_path);
        return;
    }
    fclose(f);
    LOGI("Model file is accessible, size check deferred to llama");
    ...
    if (!g_model) {
        ...
        LOGE("Model path: %s, n_gpu_layers=%lld, errno=%d: %s", model_path, (long long)n_gpu_layers, errno, strerror(errno));
        env->ReleaseStringUTFChars(path, model_path);
        jclass exception = env->FindClass("java/lang/RuntimeException");
